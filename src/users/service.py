...
def change_password(db: Session, user_id: UUID, password_change: model.PasswordChange) -> None:
    try:
        ...
        if password_change.new_password != password_change.new_password_confirm:
            logging.warning(f"New password and confirm password do not match for user {user_id}")
            raise PasswordMismatchError(f"New password and confirm password do not match for user {user_id}")
